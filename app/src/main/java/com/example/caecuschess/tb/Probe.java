...
public class Probe {
    ...
    private GtbProbeResult gtbProbe(Position pos) {
        ...
        if (ret.result == GtbProbeResult.DRAW && pos.getEpSquare() != -1) {
            ArrayList<Move> moveList = MoveGeneration.instance.legalMoves(pos);
            int pawn = pos.whiteMove ? Piece.WPAWN : Piece.BPAWN;
            int maxMate = -1;
            UndoInfo ui = new UndoInfo();
            for (Move move : moveList) {
                if ((move.SQto != pos.getEpSquare()) || (pos.getPiece(move.SQfrom) != pawn))
                    return ret;
    ...
    private GtbProbeResult gtbProbeRaw(Position pos) {
        int castleMask = 0;
        if (pos.whiteLongCastle()) castleMask |= GtbProbe.A1_CASTLE;
        if (pos.whiteShortCastle()) castleMask |= GtbProbe.H1_CASTLE;
        if (pos.blackLongCastle()) castleMask |= GtbProbe.A8_CASTLE;
        if (pos.blackShortCastle()) castleMask |= GtbProbe.H8_CASTLE;
    ...
    private ProbeResult rtbProbe(Position pos) {
        ...
        rtb.probe(squares, pos.whiteMove, pos.getEpSquare(), pos.getCastleMask(),
                  pos.DrawTimer, pos.MoveCounter, result);
        int wdl = 0;
    ...
    public final ArrayList<Move> removeNonOptimal(Position pos, ArrayList<Move> moveList) {
        ...
        for (Move m : moveList) {
            pos.makeMove(m, ui);
            int pliesToDraw = Math.max(100 - pos.DrawTimer, 1);
    ...
    public final ArrayList<Pair<Integer,ProbeResult>> movePieceProbe(Position pos, int fromSq) {
        ...
        ArrayList<Move> moveList = new MoveGeneration().legalMoves(pos);
        UndoInfo ui = new UndoInfo();
        for (Move m : moveList) {
            if (m.SQfrom != fromSq)
                continue;
            pos.makeMove(m, ui);
            boolean isZeroing = pos.DrawTimer == 0;
