...
public class Game {
    ...
    final void readFromStream(DataInputStream dis, int version) throws IOException, ChessError {
    ...
    final public boolean readPGN(String pgn, PGNOptions options) throws ChessError {
    ...
    public final Pair<Boolean, Move> processString(String str) {
        ...
        Move m = TextInfo.UCIStringToMove(str);
        if (m != null)
            if (!TextInfo.isValid(currPos(), m))
                m = null;
        if (m == null) {
            m = TextInfo.stringToMove(currPos(), str);
            if (!TextInfo.isValid(currPos(), m))
                m = null;
        }
        if (m == null)
    ...
    private boolean gameEndingMove(Move m) {
        ...
        pos.makeMove(m, ui);
        boolean gameEnd = MoveGeneration.instance.legalMoves(pos).isEmpty();
        pos.UndoMove(m, ui);
        return gameEnd;
    }
    ...
    public final Pair<Position, ArrayList<Move>> getUCIHistory() {
        ...
        for (int i = 0; i < nMoves; i++) {
            ...
            if (currPos.DrawTimer == 0 && n.move.equals(new Move(0, 0, 0))) {
    ...
    private Move handleDrawCmd(String drawCmd, boolean playDrawMove) {
        ...
        if (drawCmd.startsWith("rep") || drawCmd.startsWith("50")) {
            ...
            if (firstSpace >= 0) {
                ...
                if (ms.length() > 0) {
                    m = TextInfo.stringToMove(pos, ms);
                }
            }
            boolean valid;
            if (rep) {
                ...
                if (tmpPos.isDrawRuleEqual(posToCompare))
                    repetitions++;
                int nMoves = ml.second;
                for (int i = 0; i < nMoves; i++) {
                    Node n = moveList.get(i);
                    tmpPos.makeMove(n.move, ui);
                    TextInfo.legalEPSquare(tmpPos);
                    if (tmpPos.isDrawRuleEqual(posToCompare))
                        repetitions++;
                }
                if (repetitions >= 3)
                ...
                valid = tmpPos.DrawTimer >= 100;
            }
            if (valid) {
                String playerAction = rep ? "draw rep" : "draw 50";
                if (m != null)
                    playerAction += " " + TextInfo.moveToString(pos, m, false, false);
            ...
            if (TextInfo.stringToMove(pos, ms) != null) {
                ret = processString(ms).second;
            }
