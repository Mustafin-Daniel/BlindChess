...
import com.example.caecuschess.GameEngine.GameTree;
...
public class EcoDb {
    ...
    public Result getEco(GameTree gt) {
        ...
        while (true) {
            ...
            Short idx = posHashToNodeIdx.get(gt.currentPos.createZobristHash());
        ...
        if (nodeIdx != -1 && checkForDup && gt.startPos.createZobristHash() == startPosHash) {
            ArrayList<Short> dups = posHashToNodeIdx2.get(gt.currentPos.createZobristHash());
            if (dups != null) {
                ...
                while (!treePath.isEmpty()) {
                    ...
                    int m = gt.currentNode.move.TurnToCompressed();
    ...
    public ArrayList<Move> getMoves(Position pos) {
        ...
        long hash = pos.createZobristHash();
        ...
        if (idx != null) {
            ...
            while (child != -1) {
                node = readNode(child);
                moves.add(Move.TurnFromCompressed(node.move));
            ...
            if (lst != null) {
                for (Short idx2 : lst) {
                    ...
                    while (child != -1) {
                        node = readNode(child);
                        Move m = Move.TurnFromCompressed(node.move);
                        if (!moves.contains(m))
                            moves.add(m);
                        child = node.nextSibling;
                    }
    ...
    private EcoDb() {
        ...
        try (ByteArrayOutputStream bufStream = new ByteArrayOutputStream();
             InputStream inStream = CaecusChessApp.getContext().getAssets().open("eco.dat")) {
        ...
        try {
            ...
            startPosHash = pos.createZobristHash();
