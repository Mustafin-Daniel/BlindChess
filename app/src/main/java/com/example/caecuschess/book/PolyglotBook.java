...
class PolyglotBook implements IOpeningBook {
    ...
    static long getHashKey(Position pos) {
        ...
        // Castle flags
        if (pos.whiteLongCastle()) key ^= hashRandoms[768];
        if (pos.whiteShortCastle()) key ^= hashRandoms[768 + 1];
        if (pos.blackShortCastle()) key ^= hashRandoms[768 + 2];
        if (pos.blackLongCastle()) key ^= hashRandoms[768 + 3];
