...
//                                        TextInfo.moveToUCIString(be.move),
//                                        w0, recom, (int)score, weight);
    ...
    private static byte[] positionToByteArray(Position pos) {
        ...
        if (cs) {
            bits.addBit(pos.blackLongCastle());
            bits.addBit(pos.blackShortCastle());
            bits.addBit(pos.whiteShortCastle());
            bits.addBit(pos.whiteLongCastle());
        }
...
//            int wins  = extractInt(buf, statStart + 3, 3);
    ...
    private static Position mirrorPosColor(Position pos) {
        ...
        int castleMask = 0;
        if (pos.whiteLongCastle()) castleMask |= (1 << Position.WhiteShortCastleMask);
        if (pos.whiteShortCastle()) castleMask |= (1 << Position.WhiteLongCastleMask);
        if (pos.blackLongCastle()) castleMask |= (1 << Position.BlackShortCastleMask);
        if (pos.blackShortCastle()) castleMask |= (1 << Position.BlackShortCastleMask);
        ...
        ret.DrawTimer = pos.DrawTimer;
        ret.MoveCounter = pos.MoveCounter;
        return ret;
    }
    ...
    private static Move mirrorMoveColor(Move m) {
        ...
        Move ret = new Move(m);
        ret.SQfrom = mirrorSquareColor(m.SQfrom);
        ret.SQto = mirrorSquareColor(m.SQto);
    ...
    private static Move mirrorMoveLeftRight(Move m) {
        if (m == null) return null;
        Move ret = new Move(m);
        ret.SQfrom = mirrorSquareLeftRight(m.SQfrom);
        ret.SQto = mirrorSquareLeftRight(m.SQto);
