...
public abstract class ChessBoard extends View {
    ...
    private final class AnimInfo {
        ...
        private boolean animActive() {
            return !paused && startTime >= 0 && now < stopTime && posHash == pos.createZobristHash();
        }
...
//          System.out.printf("delay:%d\n", delay);
    ...
    public final void setAnimMove(Position sourcePos, Move move, boolean forward) {
        ...
        if (forward) {
            ...
            anim.posHash = targetPos.createZobristHash();
        } else {
            anim.posHash = sourcePos.createZobristHash();
        }
        int animTime; // Animation duration in milliseconds.
        {
            int dx = Position.getX(move.SQto) - Position.getX(move.SQfrom);
            int dy = Position.getY(move.SQto) - Position.getY(move.SQfrom);
        ...
        if (animTime > 0) {
            ...
            anim.hide2 = -1;
            if (forward) {
                int p = sourcePos.getPiece(move.SQfrom);
                anim.piece1 = p;
                anim.from1 = move.SQfrom;
                anim.to1 = move.SQto;
                anim.hide1 = anim.to1;
                int p2 = sourcePos.getPiece(move.SQto);
                if (p2 != Piece.EMPTY) { // capture
                    anim.piece2 = p2;
                    anim.from2 = move.SQto;
                    anim.to2 = move.SQto;
                } else if (move.SQto == sourcePos.getEpSquare()) {
                    if (sourcePos.whiteMove) {
                        anim.piece2 = Piece.BPAWN;
                        anim.from2 = anim.to2 = move.SQto - 8;
                    } else {
                    ...
                    if (move.SQto == move.SQfrom + 2) { // O-O
                        anim.piece2 = wtm ? Piece.WROOK : Piece.BROOK;
                        anim.from2 = move.SQto + 1;
                        anim.to2 = move.SQto - 1;
                        anim.hide2 = anim.to2;
                    } else if (move.SQto == move.SQfrom - 2) { // O-O-O
                        anim.piece2 = wtm ? Piece.WROOK : Piece.BROOK;
                        anim.from2 = move.SQto - 2;
                        anim.to2 = move.SQto + 1;
                        anim.hide2 = anim.to2;
                    }
                }
                ...
                anim.from1 = move.SQto;
                anim.to1 = move.SQfrom;
                anim.hide1 = anim.to1;
    ...
    public final void setBlindMode(boolean blindMode) {
        if (this.blindMode != blindMode) {
            ...
            PieceSet.setInst(new PieceSet());
            invalidate();
        }
    }

    public final void setUltraBlindMode(boolean ubm){
        if(this.ubm!=ubm){
            this.ubm = ubm;
            PieceSet.setInst(new PieceSet());
            invalidate();
...
//      System.out.printf("draw: %d\n", t1-t0);
    ...
    protected final void drawPiece(Canvas canvas, int xCrd, int yCrd, int p) {
        Bitmap bm;
        if(blindMode){
            bm = PieceSet.instance().getBlindPieceBitmap(p, sqSize, ubm);
        } else{
            bm = PieceSet.instance().getPieceBitmap(p, sqSize);
        }
        if (bm != null) {
