...
public class TextInfo {
    ...
    public static Position readFEN(String fen) throws ChessError {
        ...
        if (words.length < 2)
            throw new ChessError("too few pieces");
        ...
        for (int i = 0; i < words[0].length(); i++) {
            ...
            switch (c) {
                ...
                default: throw new ChessError("invalid piece", pos);
            }
        }
            ...
            for (int i = 0; i < words[2].length(); i++) {
                ...
                switch (c) {
                    ...
                    case '-':
                        break;
                    default:
                        throw new ChessError("invalid castling flags", pos);
                }
            }
        }
        ...
        if (words.length > 3) {
            ...
            if (!epString.equals("-")) {
                if (epString.length() < 2)
                    throw new ChessError("invalid en passant square", pos);
        ...
        if (nPieces[Piece.WKING] != 1)
            throw new ChessError("white num kings", pos);
        if (nPieces[Piece.BKING] != 1)
            throw new ChessError("black num kings", pos);
        ...
        if (MoveGeneration.inCheck(pos2)) {
            throw new ChessError("king capture possible", pos);
        }
    ...
    private static void safeSetPiece(Position pos, int col, int row, int p) throws ChessError {
        if (row < 0) throw new ChessError("too many rows");
        if (col > 7) throw new ChessError("too many columns");
        if ((p == Piece.WPAWN) || (p == Piece.BPAWN)) {
            if ((row == 0) || (row == 7))
                throw new ChessError("pawn on first last rank");
        }
